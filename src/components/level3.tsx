...
const CanvasTurtle3: React.FC<Props> = ({ commands, resetting }) => {
  ...
  useEffect(() => {
    ...
    const moveLoop = () => {
      ...
      // 👇 そのあとに「ちょうどゴール範囲内」を評価
      else if (isAtGoal && !goalReached && !isOverGoal) {
        setGoalReached(true);
        if (audioRef.current) {
          audioRef.current
            .play()
      ...
      {goalReached && overGoal === false && (
        <div className="fixed inset-0 z-50 flex justify-center items-center bg-neutral-900/80">
          ...
          <div className="bgOriWhite p-6 rounded-lg shadow-lg text-center max-w-md w-full">
            ...
            <p className="mb-4">やったね！ かめもゴールに到達しました。</p>
            <div
